...
cambiarContController.$inject = ['$rootScope', '$scope', '$location', 'rootRef', '$timeout', 'firebaseSvc', '$firebaseAuth', 'toaster'];

function cambiarContController($rootScope, $scope, $location, rootRef, $timeout, firebaseSvc, $firebaseAuth, toaster) {
    var vm = this;
    vm.txtEmail = '';
    vm.txtPassOld = '';
    vm.txtPassNew = '';
        ...
        rootRef.changePassword({
            ...
            if (error) {
                ...
                auth.$authWithPassword({
                    ...
                    vm.firebaseSvc.actualizarStatus(authData.uid).then(function (user) {
                        if (user.isAdmin) {
                            $location.path('/');
                            $rootScope.isAuthenticated = true;
                        } else {
                            vm.firebaseSvc.salir();
